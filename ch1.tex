...
\begin{ex}
	...
	\bigskip
	Notice that $h$ too isn't a surjection: $-1$ isn't the image of any real number under $h$. However, if we define $h':\R\to\R^{\geq0}$, where $\R^{\geq0}$ the set of all non-negative real numbers, by putting $h'(x)=h(x)$, we see that $h'$ is, now, a surjection.
\end{ex}
...
Now that that's out of the way, we need to start working towards more interesting results. For that, we'll need some definitions first.
...
\end{theorem}
\begin{proof}
	Suponha que \(f:A\to B\) é injetiva. Então dados \(g,h:C\to A\) tais que \(fg\equiv fh\), como \(f\) é injetiva, \(f(g(c))=f(h(c))\) implica \(g(c)=h(c)\), para todo \(c\in C\). Logo \(g\equiv h\) e \(f\) é monomorfismo. Reciprocamente, se \(f:A\to B\) é monomorfismo, defina as funções \(g,h:\{c\}\to A\) fazendo \(g(c)=a\) e \(h(c)=a'\) para \(a\neq a'\in A\) fixos. Então o monomorfismo de \(f\) nos garante que como \(g\not\equiv h\), temos \(fg\not\equiv fh\). Mas, então:
	\[f(a')=f(h(c))=fh(c)\neq fg(c)=f(g(c))=f(a)\]ou seja, \(a\neq a'\) nos garante que \(f(a)\neq f(a')\), logo \(f\) é injetiva.
\end{proof}

\begin{df}
	A function $f:A\to B$ is calle an \textbf{epimorphism} if given any other two functions $g,h:B\to C$, we have that $g\circ f=h\circ f$ implies $g=h$.
\end{df}

%\subsection{Operating on sets}
%
%We've defined unions and intersections as operations on sets, but they might not be what you expect. 
%\subsection{Relations are hard, and often leave us broken inside}
%
%Now that we have the power (set) we can start working with relations.
%
%\begin{df}
%	Given a set $X$
%\end{df}