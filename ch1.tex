...
\begin{restatable}[Weak Yoneda's Lemma]{theorem}{yoneda}\label{thm:yoneda1}
...
\begin{proof}
	\begin{itemize}
		...
		So if we take $f:A\to X$ any function, by applying the equality above to $\id_X$ we see that
		...
		But this shows us that $\phi_A:\hom(A,X)\to\hom(A,Y)$ is uniquely defined by $\phi_X(\id_X):X\to Y$.
		
		In other words, for each $\phi_A:\hom(A,X)\to\hom(A,Y)$ we can define a unique $\phi_X(\id_X):X\to Y$ such that $\phi_A(f)=\phi_X(\id_X)\circ f$, and for each $\varphi:X\to Y$ we can get a unique $\hom(A,\varphi):\hom(A,X)\to\hom(A,Y)$.
		
		Clearly these correspondences are mutually inverse:
		
		\begin{itemize}
			\item Take $\varphi:X\to Y$ to $\hom(A,\varphi)$, and then take that to $\hom(A,\varphi)(\id_X)$. But this follows trivially: $\hom(A,\varphi)(\id_X)=\varphi$:	
			\[\hom(A,\varphi)(\id_X)=\varphi\circ\id_X=\varphi.\]
			
			\item Take $\phi_A:\hom(A,X)\to\hom(A,Y)$ to the unique $\phi_X(\id_X)$ such that $\phi_A(f)=\phi_X(\id_X)\circ f$ for all $f\in\hom(A,X)$. Now take $\phi_X(\id_X)$ to $\hom(A,\phi_X(\id_X))$. We claim that $\phi_A=\hom(A,\phi_X(\id_X))$. But this follows trivially:
			\[\phi_A(f)=\phi_X(\id_X)\circ f=\hom(A,\phi_X(\id_X))(f)\]for every $f\in\hom(A,X)$, and so $\phi_A=\hom(A,\phi_X(\id_X))$.
		\end{itemize}
	
		Finally, if we take $\phi_A$ as an isomorphism, for every $A$, we see that $\hom(A,\phi_X(\id_X))$ is an isomorphism for every $A$ - which means that there's some function $h:\hom(A,Y)\to\hom(A,X)$ such that $h\circ\hom(A,\phi_X(\id_X))=\id_{\hom(A,X)}$ and $\hom(A,\phi_X(\id_X))\circ h=\id_{\hom(A,Y)}$.
	\end{itemize}
\end{proof}
