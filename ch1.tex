...
\begin{proof}
	\begin{enumerate}[(a)]
		...
		\item Since $f$ and $g\circ f$ are iso, there are inverses $f^{-1}$ and $(g\circ f)^{-1}$. From this, we have that $\id_A=(g\circ f)^{-1}\circ(g\circ f)$, and composing by $f^{-1}$ on the right on both sides gives us $f^{-1}=((g\circ f)^{-1}\circ(g\circ f))\circ f^{-1}$. Now we use the associativity twice to see that $f^{-1}=(g\circ f)^{-1}\circ (g\circ (f\circ f^{-1}))$ which is just $f^{-1}=(g\circ f)^{-1}\circ g$. Finally, by composing $f$ on the left on both sides of the equation we get $\id_B=(f\circ (g\circ f)^{-1})\circ g$, and so $g$ has a left-inverse.
	\end{enumerate}
\end{proof}