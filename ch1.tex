...
\begin{proof}
	...
	This means that $q(z)$ is a point in $X\times Y$ whose $X$-coordinate is $p_X(z)$ (this is what the equation $p_X(z)=\pi_X(q(z))$ tells us), and whose $Y$-coordinate is $p_Y$ (this is what the equation \(p_Y(z)=\pi_Y(q(z))\) tells us). Therefore, $q(z)=(p_X(z),p_Y(z))$ and the RHS is just $p(z)$, by definition. So we have $q(z)=p(z)$ for all $z\in Z$ - which implies, by the definition of function equality, that $q=p$.
