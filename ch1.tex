...
\begin{lemma}
	If $X\arrow{f}Y\arrow{g}Z$, then, for any $A$ we have that $\hom(g\circ f,A)=\hom(f,A)\circ\hom(g,A)$ and $\hom(g\circ f,A)=\hom(f,A)\circ\hom(g,A)$.
\end{lemma}
\begin{proof}
...
\begin{proof}
	...
	\bigskip
	Conversely, let $\phi:X\to Y$ be an isomorphism. Now, for any $A$, consider the maps $f:=\hom(A,\phi):\hom(A,X)\to\hom(A,Y)$ and $f':=\hom(A,\phi^{-1}):\hom(A,Y)\to\hom(A,X)$.
...
\begin{proof}
	...
	\begin{itemize}
		...
		let $\phi:X\to Y$ be an isomorphism. Now, for any $A$, consider the maps $f:\hom(X,A)\to\hom(Y,A)$ and $f':\hom(Y,A)\to\hom(X,A)$ given by, respectively,
		\[f(g):=g\circ\phi\]
