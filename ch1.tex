...
\begin{ex}
	...
	By definition, we must have $\id_A=\pi^1_A\circ\Delta_X$ and $\id_A=\pi^1_A\circ\Delta_X$. So take any $a\in A$.
...
\begin{lemma}
	For any two sets $X$ and $Y$ we have that $X\sqcup Y\iso (X\times\{X\})\cup (Y\times{Y})$.
\end{lemma}
\begin{proof}
	It follows directly from the example above and from the fact that we can denote any element in $X\times\{X\}$ (which is of the form $(x,X)$, by definition) as $x_X$, just as a shorthand notation (and similarly for $Y\times\{Y\}$).
	
	\bigskip
	Define $f:X\sqcup Y\to (X\times\{X\})\cup (Y\times{Y})$ by putting
	\[f(a):=\begin{cases}
	a_X,&\mbox{ if }a\in X\\
	a_Y,&\mbox{ if }a\in Y.
	\end{cases}\]
	...
	\begin{itemize}
		...
		Take $a,b\in X\sqcup Y$ such that $f(a)=f(b)$.
		
		If $a\in X$ and $b\in X$, we see that $a_X=f(a)=f(b)=b_X$ and hence $a=b$.
		
		If $a\in X$ and $b\in Y$, we see that $a_X=f(a)=f(b)=b_Y$, which is absurd because the second coordinate of $b_Y$ is $Y$, and the second coordinate of $a_X$ is $X$, and $X\neq Y$, so $b_Y\neq a_X$. So this case cannot happen.
		
		Similarly, if $a\in Y$ and $b\in X$ we'd get that same contradiction, so this case also cannot happen.
		
		Finally, if $a\in Y$ and $b\in Y$, just like the first case we can see that $a_Y=b_Y$, and hence $a=b$.
		...
		Take $c\in (X\times\{X\})\cup (Y\times{Y})$. We want to show that there's some $a\in X\sqcup Y$ such that $f(a)=c$.
		
		Since $(X\times\{X\})\cap (Y\times{Y})=\varnothing$, $c$ must lie in either $X\times\{X\}$ or $Y\times{Y}$ - that is, either $c=x_X$ for some $a\in X$ or $c=y_Y$ for some $y\in Y$. But this means that $c$ is either $f(x)$, for some $x\in X$ or $f(y)$, for some $y\in Y$.
		
		No matter which one of these hold, there's always one $a\in X\sqcup Y$ such that $f(a)=c$, for any $c\in (X\times\{X\})\cup (Y\times{Y})$. This proves that $f$ is surjective.
	\end{itemize}
...
\begin{proof}
	...
	\begin{enumerate}[(i.)]
		\item There are functions $\iota_X:X\to X\sqcup Y$ and $\iota Y:Y\to x\sqcup Y$.
