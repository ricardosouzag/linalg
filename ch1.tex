...
\begin{theorem}[Yoneda's Lemma]
	Let $X$ and $Y$ be any sets. Then any family of functions $\{\phi_A:\hom(A,X)\to \hom(A,Y)\}_A$, where $A$ ranges over all sets, determines a unique function $f:X\to Y$, and vice-versa.
\end{theorem}
\begin{proof}
	...
	In other words, giving a family of functions $\{\phi_A:\hom(A,X)\to\hom(A,Y)\}_A,$ indexed over all sets, is the same as choosing a function $f:X\to Y$ and putting every other $\phi_A:=\hom(A,f)$.
