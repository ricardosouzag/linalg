...
\begin{theorem}
	Let $A$ be a set and $\varnothing_A$ and $\emptyset_A$ be two possible empty sets for $A$. Then they are equal.
\end{theorem}
\begin{proof}
	We need to show that every element of $\varnothing_A$ is an element of $\emptyset_A$, and vice-versa.
	
	Assume that's not the case - then there's either some element of $\varnothing_A$ which is not in $\emptyset_A$, or some element of $\emptyset_A$ which is not in $\varnothing_A$. But both of these are impossible, since both sets are empty.
	
	Therefore, $\varnothing_A=\emptyset_A$
\end{proof}
\begin{cor}
...
\begin{df}
	We're going to define the unique empty set to be the empty set of any set $A$, which will be denoted in symbols by $\varnothing$.
\end{df}
