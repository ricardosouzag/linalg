...
\begin{ex}
	...
	\bigskip 
	Let, now, $f:\R\to\R$ be defined by $f(x)=x^2$. Is $f$ injective?
	
	Well, suppose $x$ and $x'$ are such that $f(x)=f(x')$. This means that $x^2=x'^2$. Taking square roots on both sides we get that $\lvert x\rvert=\lvert x'\rvert$ - which can be further simplified to mean $x=\pm x'$. In other words, we see that if two points have the same square, then they must differ only by a sign. That's good and all, but also shows us that two numbers that differ by a sign have the same image under $f$ - and therefore $f$ cannot be injective.
	
	For instance, $2\neq -2$, but $f(2)=f(-2)=4$.
\end{ex}

\begin{df}
	A $f:A\to B$ function is called a \textbf{surjection} if for any $b\in B$ there is some $a\in A$ such that $f(a)=b$.
\end{df}
