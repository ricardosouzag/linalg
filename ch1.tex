...
\begin{lemma}
	A set $X$ is finite if, and only if, there is no surjection from $X$ to $\N$.
\end{lemma}
\begin{proof}
	Suppose there is a surjection $f:X\to \N$.
	
	Since $f$ is a surjection, we know that it has a right-inverse $g:\N\to X$ such that $\id_\N=f\circ g$.
	
	But since $\id_\N$ is an isomorphism, and $f\circ g$ is a composition that is an isomorphism, we get that $g$ is injective.
\end{proof}