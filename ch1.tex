...
\begin{proof}
	\begin{itemize}
		...
		Take $f:B\to A$ any function, and look at $\phi:\hom(A,X)\to\hom(A,Y)$ and $\varphi:\hom(B,X)\to\hom(B,Y)$, isomorphisms.
		
		Assume that $\hom(f,Y)\circ\phi=\varphi\circ\hom(f,X)$ holds for any choice of $A$, $B$ and $f$. So if we take $f:Y\to X$ any function, by applying the equality above to $\id_X$ we see that
		\[(\hom(f,Y)\circ\phi)(\id_X)=\hom(f,Y)(\phi(\id_X))=(\phi(\id_X))\circ f\]and
		\[(\varphi\circ\hom(f,X))(\id_X)=\varphi(\hom(f,X)(\id_X))=\varphi(\id_X\circ f)=\varphi(f)\]must be equal - in other words, $\phi(\id_X)\circ f=\varphi(f)$.
