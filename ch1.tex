...
\end{cor}
\begin{proof}
	We will only prove (i.) implies (iii.). (ii.) implies (iii.) is similar, and (iii.) implies both (i.) and (ii.) is trivial. Those are left as an exercise to the reader.
	...
	\bigskip
	Conversely, if $f:X\to Y$ is an isomorphism, then clearly $\{\phi_A:=\hom(A,f)\}_A$ is a family of isomorphisms which commute the diagrams.
