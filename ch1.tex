...
\begin{blockenv}{Definition}
	...
	\begin{enumerate}[i.]
		\item $p(X)$;
		\item If there is some $Y$ such that $p(Y)$, then there is a function $f_{p}:X\to Y$;
		\item If any $Z$ satisfies the two above conditions, then there is a function $g:Z\to X$ such that all the functions $g_{p}:Z\to Y$ (that exist, since $Z$ satisfies (ii.)) are of the form $g_{p}=f_{p}\circ g$.
	\end{enumerate}
\end{blockenv}