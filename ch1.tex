...
\bigskip
Conversely, suppose $X$ is such that there's no surjection from $X$ to $\N$. Take any such function $f:X\to \N$ and consider its image $\im(f)$.

Now, we can induce a new function $f':X\to\im(f)$ by putting $f'(x):=f(x)$ for all $x\in X$. It is clear that $f'$ is surjective, by definition. So it has a right-inverse, $g:\im(f)\to X$ such that $\id_{\im(f)}=f\circ g$.
\end{proof}