...
Well, I don't know. Assume that $N\notin N$. But since $N$ is, by definition, the set of all non-weird sets, saying that $N\notin N$ is the same as saying that $N$ is weird. But then - being weird means that $N\in N$... Which contradicts our assumption that $N\notin N$.

Okay, maybe we made a mistake all along by assuming that $N\notin N$! Yeah, that must be the case! Clearly, $N$ can't be a weird set!... But then, since $N$ isn't weird, it must be an element of $N$ (since $N$ contains \textit{all} non-weird sets)... So it is weird. So if we assume $N\in N$ we can logically deduce that $N\notin N$.
...
\end{rmk}
\newpage
\section{About functions, relations and cardinality}
...
\newpage
\subsection{Bijection as an equality}
...
\newpage
\subsection{Relations, order and quotients}
...
\begin{theorem}[Cantor-Bernstein-Schroeder]\label{thm:cantor bernstein schroeder}
...
\newpage
\subsection{About sets of functions}
...
\begin{theorem}[Sets are cartesian-closed]\label{thm:tensor-hom}
...
\begin{df}
	Let $X,Y$ be sets. A \textbf{natural transformation $\phi:\hom(-,X)\to\hom(-,Y)$} is a family of functions $\phi:=\{\phi_A:\hom(A,X)\to \hom(A,Y)\}_A$, where $A$ ranges over all sets, such that 
	\[\begin{tikzcd}
	{\hom(B,X)} \arrow[r, "\phi_B"] \arrow[d, "{\hom(g,X)}"'] & {\hom(B,Y)} \arrow[d, "{\hom(g,Y)}"] \\
	{\hom(A,X)} \arrow[r, "\phi_A"']                          & {\hom(A,Y)}                         
	\end{tikzcd}\]commutes for all $g:A\to B$.
	
	If each $\phi_A$ in the family is an isomorphism, then we say that $\phi$ is a \textbf{natural isomorphism}.
\end{df}

\begin{theorem}[Yoneda's Lemma]\label{thm:yoneda}
	Let $X$ and $Y$ be any sets. Then there is a bijective correspondence between natural transformations $\phi:\hom(-,X)\to\hom(-,Y)$, and functions $f:X\to Y$.
\end{theorem}
\begin{proof}
	Take $g:A\to B$ any function.
	
	Since the diagram commutes, by hypothesis, $\hom(g,Y)\circ\phi_B=\phi_A\circ\hom(g,X)$ holds for any choice of $A$, $B$ and $g$.
	
	So if we take $g:A\to X$, by applying the equality above to $\id_X$ we see that
	\[(\hom(g,Y)\circ\phi_X)(\id_X)=\hom(g,Y)(\phi_X(\id_X))=(\phi_X(\id_X))\circ g\]and
	\[(\phi_A\circ\hom(g,X))(\id_X)=\phi_A(\hom(g,X)(\id_X))=\phi_A(\id_X\circ g)=\phi_A(g)\]must be equal - in other words, $\phi_X(\id_X)\circ g=\phi_A(g)$.
...
\end{proof}
\begin{df}
	Let $X,Y$ be sets. A \textbf{natural transformation $\phi:\hom(X,-)\to\hom(Y,-)$} is a family of functions $\phi:=\{\phi_A:\hom(X,A)\to \hom(Y,A)\}_A$, where $A$ ranges over all sets, such that \[\begin{tikzcd}
	...
	If each $\phi_A$ in the family is an isomorphism, then we say that $\phi$ is a \textbf{natural isomorphism}.
\end{df}
\begin{cor}[Yoneda's Lemma]
	Let $X$ and $Y$ be any sets. Then there is a bijective correspondence between natural transformations $\psi:\hom(X,-)\to\hom(Y,-)$, and functions $f:Y\to X$.
\end{cor}
\begin{proof}
	It's essentially the same proof as the theorem's, so it'll be left as an exercise to the reader.
\end{proof}
\begin{cor}
	For any pair of sets $X,Y$ the following are equivalent:
	\begin{enumerate}[(i.)]
		\item There is a natural isomorphism $\phi:\hom(-,X)\to\hom(-,Y)$;
		\item There is a natural isomorphism $\psi:\hom(Y,-)\to\hom(X,-)$;
		\item $X\iso Y$.
	\end{enumerate}
\end{cor}
\begin{proof}
	...
	\bigskip
	Let $\phi:=\{\phi_A:\hom(A,X)\to\hom(A,Y)\}_A$ be a natural isomorphism. Analogously, let $\phi^{-1}:=\{\phi^{-1}_A:\hom(A,Y)\to\hom(A,X)\}_A$ be the inverse natural isomorphism.
	
	In light of \hyperref[thm:yoneda]{Yoneda's Lemma}, we see that $\phi$ and $\phi^{-1}$ determine unique functions $f_\phi:X\to Y$ and $f_{\phi^{-1}}:Y\to X$ given by $f_\phi:=\phi_X(\id_X)$ and $f_{\phi^{-1}}:=\phi^{-1}_Y(\id_Y)$, respectively, such that $\phi_A=\hom(A,f_{\phi})$ and $\phi^{-1}_A=\hom(A,f_{\phi^{-1}})$ for every set $A$.
	
	We claim that they are mutually inverse and, therefore, $X\iso Y$.
	
	To see this, let us compute:
	\begin{align*}
	\hom(X,\id_X)&=\id_{\hom(X,X)}\\
	&=\phi^{-1}_X\circ\phi_X\\
	&=\hom(X,f_{\phi^{-1}})\circ\hom(X,f_{\phi})\\
	&=\hom(X,f_{\phi^{-1}}\circ f_{\phi})
	\end{align*}
	and
	\begin{align*}
	\hom(Y,\id_Y)&=\id_{\hom(Y,Y)}\\
	&=\phi_Y\circ\phi^{-1}_Y\\
	&=\hom(Y,f_{\phi})\circ\hom(Y,f_{\phi^{-1}})\\
	&=\hom(Y,f_{\phi}\circ f_{\phi^{-1}})
	\end{align*}together tells us that $f_{\phi}$ and $f_{\phi^{-1}}$ are mutually inverse and, therefore, isomorphisms.
	
	This proves $X\iso Y$.
	
	\bigskip
	Conversely, if $f:X\to Y$ is an isomorphism, then clearly $\phi:=\{\phi_A:=\hom(A,f)\}_A$ is a natural isomorphism.
	
	We have thus shown that (i.) if, and only if, (iii.), which ends the proof.
\end{proof}

Summing it all up, what Yoneda's Lemma tells us is that the only possible natural transformations are of the form $\hom(-,f):\hom(-,X)\to\hom(-,Y)$ or $\hom(f,-):\hom(Y,-)\to\hom(X,-)$, for some $f:X\to Y$.
...
\begin{theorem}
	The isomorphism in \cref{thm:tensor-hom} is natural for all three entries - that is, we have three natural isomorphisms:
	\[\phi_{X,Z}:\hom(X\times-,Z)\to\hom(X,\hom(-,Z))\]
	\[\phi_X^Y:\hom(X\times Y,-)\to\hom(X,\hom(Y,-))\]
	\[\phi_Z^Y:\hom(-\times Y,Z)\to\hom(-,\hom(Y,Z))\]
\end{theorem}
