...
\begin{df}
	...
	\begin{enumerate}[i.]
		\item There is a function $\pi_n:\prod_{i\in I}A_i\to A_n$ of functions from $\prod_{i\in I}A_i$ to $A_n$, for each $n\in I$;
		\item If $Z$ is a set with a function $p_n:Z\to A_n$ from $Z$ to $A_n$, for each $n\in I$, then there's a unique function $p:Z\to\prod_{i\in I}A_i$ such that we have a set of equalities $\{p_n=\pi_n\circ p\}_{n\in I}$.
	\end{enumerate}
\end{df}
