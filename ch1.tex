...
\begin{proof}
	\begin{enumerate}[(a)]
		...
		\item Let $\pi_A:A\times(B\times C):A$, $\pi_{B\times C}:A\times(B\times C)\to B\times C$, $\pi_B:B\times C\to B$, $\pi_C:B\times C\to C$, $\pi_{A\times B}:(A\times B)\times C\to A\times B$, $\pi'_A:A\times B\to A$, $\pi'_B:A\times B\to B$ and $\pi'_C:(A\times B)\times C\to C$ be the functions given by the definition of all products involved.
	\end{enumerate}
\end{proof}
