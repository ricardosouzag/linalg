...
\begin{ex}
	...
	But if you think about it, there are other relations you could make using those two sets: ``$B$ contains one of the elements of $A$'', ``$B$ contains the quadruple of an element of $A$'', ``$B$ contains some element of $A$ added with 3" etc.
	
	You can literally make as many relations as you want to.
\end{ex}

\begin{df}
	Let $A$ and $B$ be sets. We define the \textbf{set of relations between $A$ and $B$} to be the set $\mc P(A\times B)$.
	
	In other words, a \textbf{relation between $A$ and $B$} is just a subset of $A\times B$.
\end{df}