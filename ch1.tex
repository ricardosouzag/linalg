...
\begin{proof}
	...
	Define $N=\{x\in A\cap B\mid x\notin A\mbox{ or }x\notin B\}$ the collection of all elements of $A\cap B$ which are not in either $A$ or $B$ (it can be in either one of them, but not in both at once) - which is, by definition, a subset of $A\cap B$.
	
	We can now define $U=\{x\in A\cap B\mid x\notin N\}$ the collection of all elements of $A\cap B$ which are not in $N$ - which is, by definition, a subset of $A\cap B$.
	
	We claim that $U$ is contained $A$ and $B$ as a subset. This is easy to see: Take $y$ in either $A$ or $B$ (doesn't matter which). Since $A\cup B$ contains both of them, $y\in A\cup B$. But since $x$ came from either $A$ or $B$, it cannot be in $N$ (by definition of $N$) - so it must be in $U$ (by definition of $U$). It follows that both $A$ and $B$ are contained in $U$.
