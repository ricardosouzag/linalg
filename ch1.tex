...
\begin{df}[Peano's Axioms]
	...
	\begin{enumerate}
		\item[PA1] There is a point $0\in N$ called \textit{zero}.
		\item[PA2] There is an injective function $s:N\to N$ called \textit{successor} such that $0\notin \im(s)$;
		\item[PA3] If $X$ is a set containing $0$ such that for every $x\in X$ we have that $s(x)\in X$, then $X\iso N$. 
	\end{enumerate}
