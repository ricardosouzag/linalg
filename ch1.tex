...
\begin{lemma}\label{lem: mono left-inverse}
	Every monomorphism $f:A\to B$ has a left-inverse, that is, a function $g:B\to A$ such that $\id_A=g\circ f$.
\end{lemma}
\begin{proof}
	...
	\[(g\circ f)(x)=g(f(x))=f^{-1}(f(x))=x\]since $f(x)\in f(A)$ for all $x\in A$. Therefore, we have shown that for all $x$ we have $(g\circ f)(x)=x=\id_A(x)$ - which implies, by the definition of function equality, that $g\circ f=\id_A$.
	
	This finishes the proof.
\end{proof}

\begin{lemma}\label{lem: epi right-inverse}
	Every epimorphism $f:A\to B$ has a right-inverse, that is, a function $g:B\to A$ such that $\id_B=f\circ g$.
\end{lemma}
...
\begin{proof}
	...
	Take now any bimorphism $f:A\to B$. By \cref{lem: mono left-inverse,lem: epi right-inverse} we see that $f$ has both a left- and a right-inverse. But now, \cref{lem:left- = right-inverse} shows us that $f$ is an isomorphism, which ends the proof.
\end{proof}
