...
\begin{proof}
	\begin{itemize}
		...
		In light of \hyperref[thm:yoneda]{Yoneda's Lemma}, we see that $\mc F$ and $\mc F^{-1}$ determine unique functions $f_\mc F:X\to Y$ and $f_{\mc F^{-1}}:Y\to X$ given by $f_\mc F:=\phi_X(\id_X)$ and $f_{\mc F^{-1}}:=\psi_Y(\id_Y)$, respectively, such that $\phi_A=\hom(A,f_{\mc F})$ and $\psi_A=\hom(A,f_{\mc F^{-1}})$ for each set $A$. We claim that they are mutually inverse and, therefore, $X\iso Y$.
		
		To see this, let us compute:
		\begin{align*}
			f_\mc F\circ f_\mc{F^{-1}}&=\phi_X(\id_X)\circ f_{\mc F^{-1}}\\
			&=\phi_Y(\mc F^{-1})\\
		\end{align*}
