...
\end{prop}
\begin{proof}
	Once again, we'll proceed by using (PA3):
	\begin{itemize}
		\item It is trivially true for $n=0$;
		\item Suppose it's true for $n$. We'll show that it's also true for $n+1$.
		
		Let $f:n+1\to n+1$ be a surjection. Take $x,y\in n+1$ such that $f(x)=f(y)$. In that case, define $g:n+1\setminus\{x\}\to n+1$ by putting $g(m):=f(m)$ for all $m\in n+1\setminus\{x\}$. Now, since we only removed $x$ from the domain and $f(x)=f(y)=g(y)$, we see that $\im(f)=\im(g)$. But $f$ is surjective, so $\im(f)=n+1$. Hence, $\im(g)=n+1$ and $g$ is surjective as well.
	\end{itemize}
\end{proof}