...
\begin{proof}
	...
	\bigskip
	Conversly, assume that $n\notin m$. Then either $m\in n$ or $n=m$ (by definition of $\N$).
	
	\begin{itemize}
		\item If $m\in n$, then, by the first case, we see that there's an injection $g:m\to n$ that is not surjective.
		
		Now, if there was an injection $f:n\to m$, then their composite $g\circ f:n\to n$ would be an injection too and so, by the preceding lemma, $g\circ f$ would be a bijection, which, by \cref{lem:mono epi iso properties}, implies that $g$ would be a surjection, contradicting the fact that we have proven that $g$ cannot be a surjection.
