...
\end{theorem}
\begin{proof}
	We need to show that every element of $\varnothing_A$ is an element of $\varnothing_B$, and vice-versa.
	
	Assume that's not the case - then there's either some element of $\varnothing_A$ which is not in $\varnothing_B$, or some element of $\varnothing_B$ which is not in $\varnothing_A$. But both of these are impossible, since both sets are empty.
	
	Therefore, $\varnothing_A=\varnothing_B$
\end{proof}
\begin{cor}
