...
Similarly, as we've seen above, inverses are also universal properties.

Here's the advantage of working with universal properties:

\begin{blockenv}{Definition}
	We say that a set $X$ is \textbf{initial} with regards to the property $\mc p$ if
	\begin{enumerate}[i.]
		\item $\mc p(X)$;
		\item If there is some $Y$ such that $\mc p(Y)$, then there is a function $f_{\mc p}:X\to Y$;
		\item If any $Z$ satisfies the two above conditions, then there is a function $g:Z\to X$ such that all the functions $g_{\mc p}:Z\to Y$ (that exist, since $Z$ satisfies (ii.)) are of the form $g_{\mc p}=f_{\mc p}\circ g$.
	\end{enumerate}
\end{blockenv}