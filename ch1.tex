...
\begin{proof}
	...
	\begin{itemize}
		...
		\item If, however, there is some $x\in n$ such that $f(x)=n$, then we know that this $x$ is unique since $f$ is injective. This allows us, then, to define a new function $g:n+1\to n+1$ as such: $g(x):=f(n)$, $g(n):=f(x)$, $g(m):f(m)$ where $x$ is the one we fixed above, and $m\in n$.
	\end{itemize}
