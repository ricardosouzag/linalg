...
\begin{restatable}[Weak Yoneda's Lemma]{theorem}{yoneda}
...
\begin{proof}
	\begin{itemize}
		...
		In light of \hyperref[thm:yoneda]{Yoneda's Lemma}, we see that $\mc F$ and $\mc F^{-1}$ determine unique functions $f_\mc F:X\to Y$ and $f_{\mc F^{-1}}:Y\to X$ given by $f_\mc F:=\phi_X(\id_X)$ and $f_{\mc F^{-1}}:=\psi_Y(\id_Y)$, respectively. We claim that they are mutually inverse and, therefore, $X\iso Y$.
		
		To see this, let us call $f:=f_{\mc F}$ and $g:=f_{\mc F^{-1}}$. Then, $f\circ g:Y\to Y$ and $g\circ f:X\to X$ also define, once again via \hyperref[thm:yoneda]{Yoneda's Lemma}, unique families of functions $\{\rho_A:\hom(A,Y)\to \hom(A,Y)\}_A$ and $\{\lambda_A:\hom(A,X)\to\hom(A,X)\}_A$ defined by $\rho_A=\hom(A,f\circ g)$ and $\lambda_A=\hom(A,g\circ f)$, respectively, for every set $A$.
