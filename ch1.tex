...
\begin{proof}
	...
	Let $f:s(X)\to s(X)$ be an injective function, and consider $f(s(X))$ its image. Now, since $f$ is injective, we have that $f(s(X))\iso s(X)$, and since $X\iso n$, we have that $s(X)\iso n+1$, and, therefore, $f(s(X))\iso n+1$ - and hence it is also finita.
\end{proof}