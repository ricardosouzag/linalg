...
\begin{proof}
	...
	Let $f:n+1\to n+1$ be an injective function, and consider $f(n)$. 
	
	If $f(n)=m\in n+1$, we consider the function $f':n\to n+1\setminus\{m\}$ defined by $f'(x):=f(x)$. Since $f$ is injective, $f'$ also is, and, therefore, $\im(f')\iso n$. But $n+1\setminus{m}\iso n$, so $f'$ can be thought of as an injection from $n$ to itself. By assumption, we see that $f'$ is surjective.
