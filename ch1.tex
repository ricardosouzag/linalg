...
\begin{proof}
	...
	Since $\hom(A,X)\iso\hom(A,Y)$ for all $A$, in particular $\hom(X,X)\iso\hom(X,Y)$. Let $\phi:\hom(X,X)\to\hom(X,Y)$ be one such isomorphism. Now consider the map $f:=\phi(\id_X):X\to Y$. We claim that $f$ is an isomorphism.
	
	\begin{itemize}
		\item $f$ is injective:
		
		Take any $x,x'\in X$ such that $f(x)=f(x')$.
	\end{itemize}
\end{proof}