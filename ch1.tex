...
\begin{lemma}
	Let $A\arrow{f}B\arrow{g}C$ be two functions such that $g\circ f$ is injective (resp. surjective). Then $f$ is also injective (resp. $g$ is also surjective).
\end{lemma}
\begin{proof}
	If $g\circ f$ is injective, by \cref{lem: mono left-inverse}, we see that there is some function $h:C\to A$ that is a left-inverse to $g\circ f$ - that is, $\id_A=h\circ(g\circ f)$. But now, by \cref{prop: composition is associative}, we see that $h\circ(g\circ f)=(h\circ g)\circ f$ and, therefore, $\id_A=(h\circ g)\circ f$ and we see that $h\circ g$ is a left-inverse for $f$. 
\end{proof}