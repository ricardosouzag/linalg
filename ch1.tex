...
\begin{proof}
	\begin{itemize}
		...
		\[(\hom(f,Y)\circ\phi)(\id_X)=\hom(f,Y)(\phi_A(\id_X))=(\phi_A(\id_X))\circ f\]and
		\[(\varphi\circ\hom(f,X))(\id_X)=\phi_B(\hom(f,X)(\id_X))=\phi_B(\id_X\circ f)=\phi_B(f)\]must be equal - in other words, $\phi_A(\id_X)\circ f=\phi_B(f)$.
		
		But this shows us that $\phi_B:\hom(B,X)\to\hom(B,Y)$ is uniquely defined by $\phi_A(\id_A)$.
	\end{itemize}
\end{proof}
