...
\begin{proof}
	...
	This allows us to define $f':X'\to f(X')$ by putting $f'(x'):=f(x')$, for all $x'\in X'$. Now this function is clearly injective and, therefore, since we're assuming the result holds true for $X$ (which is isomorphic to $X'$) we can conclude that $f'$ is surjective.
	
	But $X'=s(X)\setminus\{x\}$ and the above discussion shows that $f(X')=s(X)\setminus\{f(x)\}$. So writing
	\[f:X'\sqcup\{x\}\to f(X')\sqcup\{f(x)\}\]we see that $f$ is surjective, which finishes the proof.
\end{proof}