...
\begin{proof}
	...
	In light of \hyperref[thm:yoneda]{Yoneda's Lemma}, we see that $\phi$ and $\phi^{-1}$ determine unique functions $f_\phi:X\to Y$ and $f_{\phi^{-1}}:Y\to X$ given by $f_\phi:=\phi_X(\id_X)$ and $f_{\phi^{-1}}:=\phi^{-1}_Y(\id_Y)$, respectively, such that $\phi_A=\hom(A,f_{\phi})$ and $\phi^{-1}_A=\hom(A,f_{\phi^{-1}})$ for every set $A$. We claim that they are mutually inverse and, therefore, $X\iso Y$.
