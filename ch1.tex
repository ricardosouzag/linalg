...
Let us show some examples to build some intuition:
...
\end{prop}
\begin{proof}
	First, take any such $C$, and look at the set $C_\phi$ - that is, the set of all elements $c\in C$ such that $\phi(c)$. We claim that $C_\phi=A\cup B$.
	
	To see that $A\cup B\subseteq C_\phi$ is easy: It suffices to show that $A$ and $B$ are subsets of $C_\phi$, and then we can use (b) from the definition of the set union.
	
	To show that $A$ (resp. $B$) is a subset of $C_\phi$ we'll just use the definition: Take $a\in A$ (resp. $b\in B$). Since $A\subseteq C$ (resp. $B\subseteq C$) we have that $a\in C$ (resp. $b\in C$). We then ask: is $\phi(a)$ (resp. $\phi(b)$) true? Well, it trivially is - $\phi(x)$ is true if, and only if $x$ is in $A$ or $B$ - and $a$ (resp. $b$) certainly is. Therefore, for any $a\in A$ (resp. $b\in B$) we can conclude $\phi(a)$ (resp. $\phi(b)$) - and therefore, $a\in C_\phi$ (resp. $b\in C_\phi$). This shows that $A\subseteq C_\phi$ (resp. $B\subseteq C_\phi$).
\end{proof}
