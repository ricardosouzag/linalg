...
\begin{prop}
	Let $A$ and $B$ be two sets. Then $A=B$ is, and only if, $A\subseteq B$ and $B\subseteq A$.
\end{prop}
\begin{proof}
...
\begin{rmk}
	...
	That's what we did in the preceding proposition: If $a$=``$A=B$" and $b$=``$A\subseteq B$ and $B\subseteq A$", we proved that assuming $a$ we can conclude $b$, and that assuming $b$ we can conclude $a$.
\end{rmk}