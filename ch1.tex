...
Finally, we finish the proof of \hyperref[thm:yoneda]{Yoneda's Lemma}:

\begin{lemma}
	Let $\{\phi_A:\hom(A,X)\to\hom(A,Y)\}_A$ be a family of functions ranging over all sets. Then for any $f:A\to B$ we have that
	\[\hom(f,Y)\circ\phi_B=\phi_A\circ\hom(f,X)\]in other words, the following diagram commutes:
	\[\begin{tikzcd}
	{\hom(B,X)} \arrow[r, "\phi_B"] \arrow[d, "{\hom(f,X)}"'] & {\hom(B,Y)} \arrow[d, "{\hom(f,Y)}"] \\
