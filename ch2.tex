...
\begin{df}
	A set $\{x_1,x_2\}\subseteq \R^2$ is called a \textbf{base of $\R^2$} or a \textbf{basic set} if $$\hom(\{x_1,x_2\},\R^2)\iso\hom_\R(\R^2,\R^2)$$ that is, every function $f:\{x_1,x_2\}\to \R^2$ extends uniquely to a linear function $f':\R^2\to\R^2$ given by $f'(a,b):=af(x_1)+bf(x_2)$.
\end{df}
...
\begin{itemize}
	...
	Assume $X=\{x_1,x_2\}$ is a base for $\R^2$ and consider the canonical inclusion function $i:X\to \R^2$ given by $i(x):=x$ for all $x\in X$. Then, since $X$ is a base, it induces a unique linear transformation $i':\R^2\to\R^2$ by putting $i'(a,b):=ai(x_1)+bi(x_2)$.
\end{itemize}
\end{proof}