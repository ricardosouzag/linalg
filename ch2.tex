...
\begin{df}
	A finite set $X=\{x_1,x_2,\cdots, x_n\}\subseteq \R^2$ is called a \textbf{base of $\R^2$} or a \textbf{basic set} if $$\hom(X,\R^2)\iso\hom_\R(\R^2,\R^2)$$ that is, every linear function $f:\R^2\to\R^2$ is of the form $f(a,b)=\lambda_1f(x_1)+\lambda_2f(x_2)+\cdots+\lambda_n f(x_n)$ for some $\lambda_1,\lambda_2,\cdots,\lambda_n\in R$ which are uniquely defined for each $(a,b)\in \R^2$.
\end{df}
...
\begin{itemize}
	...
	On the other hand, applying $\id_{\R^2}$ to any $x_i\in X$ gives us
	\[x_i=\lambda_1x_1+\lambda_2x_2+\cdots+\lambda_ix_i+\cdots+\lambda_nx_n.\] But $\id_{\R^2}(x_i)=1\cdot\id_{\R^2}(x_1)$, and by definition of base, there's only one possible choice of $\lambda_j$'s. This shows that the only possible choice of $\lambda_i$ to make that true is taking $\lambda_i=1$ and every other $\lambda_j=0$ if $j\neq i$.
	
	This tells us that $x_i$ is not spanned by the other $x_j$'s, no matter which $x_i\in X$ you choose to start with.
...
This shows that (a) and (c) are equivalent, and since (c) is already equivalent to (b) and (d) this suffices.
