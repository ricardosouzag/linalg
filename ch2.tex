...
Finally, we can prove the result that we've all been waiting for:

\begin{theorem}
	Let $E_*$ be any pointed Euclidean plane. Then $E_*\iso \R^2$.
\end{theorem}
\begin{proof}	
	Let $f:E_*\to \R^2$ be the function defined as follows:
	
	First, draw two perpendicular lines through $*$ - let them be $r$ and $s$. Now, for any point $P\in E_*$, you can consider its perpendicular shadow in both $r$ and $s$ - the points $P_r$ and $P_s$. Finally, we can measure the distances $d(*,P_r)$ and $d(*,P_s)$.
	
	Then we put $f(P):=(d(*,P_r),d(*,P_s))$ for every $P\in E_*$.
	
	First thing to notice that this is indeed a function: Every step of the construction is well-defined, and given a point $P\in E_*$ and the two fixed lines $r$ and $s$, the values of $d(*,P_r)$ and $d(*,P_s)$ are uniquely defined.
	
	Now, let us prove that this function is the bijection we're looking for.
	
	\[\definecolor{uuuuuu}{rgb}{0.26666666666666666,0.26666666666666666,0.26666666666666666}
	\definecolor{ududff}{rgb}{0.30196078431372547,0.30196078431372547,1.}
	\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
	\clip(-0.2,0.5) rectangle (9.,7.);
	\draw [line width=2.pt,domain=-0.2:9.] plot(\x,{(--28.1568-2.08*\x)/8.92});
	\draw [line width=2.pt,domain=-0.2:9.] plot(\x,{(--1.8632--8.92*\x)/2.08});
	\draw [line width=2.pt,dotted,domain=-0.2:9.] plot(\x,{(-58.4136--8.92*\x)/2.08});
	\draw [line width=2.pt,dotted,domain=-0.2:9.] plot(\x,{(-51.9936--2.08*\x)/-8.92});
	\begin{scriptsize}
