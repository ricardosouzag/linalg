...
\begin{df}
	A finite set $\{x_1,x_2,\cdots, x_n\}\subseteq \R^2$ is called a \textbf{base of $\R^2$} or a \textbf{basic set} if $$\hom(\{x_1,x_2,\cdots,x_n\},\R^2)\iso\hom_\R(\R^2,\R^2)$$ that is, every linear function $f:\R^2\to\R^2$ is of the form $f(a,b)=\lambda_1f(x_1)+\lambda_2f(x_2)+\cdots+\lambda_n f(x_n)$ for some $\lambda_1,\lambda_2,\cdots,\lambda_n\in R$.
\end{df}
...
\begin{itemize}
	...
	Let $X=\{x_1,x_2\}$ be a base, and take the linear function $\id_{\R^2}:\R^2\to \R^2$. Since $X$ is a base, we know that for all $v\in \R^2$, $\id_{\R^2}(v)=\lambda_1\id_{\R^2}(x_1)+\lambda_2\id_{\R^2}(x_2)$.
	
	But $\id_{\R^2}$ is the identity function, so this becomes
	\[v=\lambda_1x_1+\lambda_2x_2\]which tells us that any $v\in \R^2$ is spanned by $X$ - in other words, $X$ spans $\R^2$. Therefore, since it is a spanning set with two elements, it must be linearly independent (since (b), (c) and (d) are already equivalent).
