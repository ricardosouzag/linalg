...
\end{prop}
\begin{proof}
	Let $f:M_{2\times 1}(\R)\to\R^2$ be given by $f\begin{pmatrix}
	x\\y
	\end{pmatrix}:=(x,y)$. Then $f$ is clearly a bijection.
\end{proof}

\begin{lemma}
	The set $M_{2\times 1}(\R)$ of all $2\times 1$ has an addition $A:M_{2\times 1}(\R)\times M_{2\times 1}(\R)\to M_{2\times 1}(\R)$ and a scalar multiplication $M:\R\times M_{2\times 1}(\R)$ satisfying:
	\begin{multicols}{2}
		\begin{itemize}
			\item $A$ is associative;
			\item $A$ is commutative;
			...
			\item $M$ has an identity element;
			\item $M$ distributes over $A$;
			\item $M$ distributes over real number addition.
		\end{itemize}
	\end{multicols}
\end{lemma}
\begin{proof}
	It suffices to define $X+Y:=f^{-1}(f(X)+f(Y))$ and $\lambda X:=f^{-1}(\lambda f(X))$, for all $X,Y\in M_{2\times 1}(\R)$ and $\lambda\in \R$.
	
	Indeed, for all $X,Y,Z\in M_{2\times 1}(\R)$ and all $\lambda,\mu\in \R$ we have:
	\begin{itemize}
		\item \begin{align*}
			(X+Y)+Z&=f^{-1}(f(X)+f(Y))+Z\\
		...
		\item Let $0:=f^{-1}(0,0)$. Then $f(0)=(0,0)$, so\[X+0=f^{-1}(f(X)+f(0))=f^{-1}(f(X))=X\]so $0$ is the identity element of the addition;
		
		\item Let $-X:=f^{-1}(-f(X))$. Then \begin{align*}
			f(X)+f(-X)&=f(X)+f(f^{-1}(-f(X)))\\
			&=f(X)-f(X)=(0,0)=f(0),
		\end{align*}so \begin{align*}
			X+(-X)&=f^{-1}(f(X)+f(-X))\\&=f^{-1}(f(0))=0
		\end{align*}and $-X$ is the additive inverse of $X$;
		
		\item \begin{align*}
			\lambda(\mu X)&=\lambda(f^{-1}(\mu f(X)))\\
			&=f^{-1}(\lambda f(f^{-1}(\mu f(X))))\\
			&=f^{-1}(\lambda (\mu f(X)))\\
			&=f^{-1}((\lambda\mu)f(X))=(\lambda\mu) X
		\end{align*}so scalar multiplication is associative;
		
		\item $\lambda X=f^{-1}(\lambda f(X))=f^{-1}(f(X)\lambda)=X\lambda$ so scalar multiplication is commutative;
		
		\item $1X=f^{-1}(1f(X))=f^{-1}(f(X))=X$ so $1$ is the identity of the scalar multiplication;
		
		\item \begin{align*}
			\lambda(X+Y)&=\lambda f^{-1}(f(X)+f(Y))\\
			&=f^{-1}(\lambda f(f^{-1}(f(X)+f(Y))))\\
			&=f^{-1}(\lambda (f(X)+f(Y)))\\
			&=f^{-1}(\lambda f(X)+\lambda f(Y))\\
			&=f^{-1}(f(f^{-1}(\lambda f(X))+f(f^{-1}(\lambda f(Y)))))\\
			&=f^{-1}(\lambda f(X))+f^{-1}(\lambda f(Y))\\
			&=\lambda X+\lambda Y
		\end{align*}so scalar multiplication distributes over addition;
		
		\item \begin{align*}
			(\lambda+\mu)(X)&=f^{-1}((\lambda+\mu)f(X))\\
			&=f^{-1}(\lambda f(X)+\mu f(X))\\
			&=f^{-1}(f(f^{-1}(\lambda f(X)))+f(f^{-1}(\mu f(X))))\\
			&=f^{-1}(\lambda f(X))+f^{-1}(\mu f(X))\\
			&=\lambda X+\mu X
...
\begin{prop}
	For each fixed base $B\subseteq \R^2$ of $\R^2$, there is a bijection $M_{2\times 2}(\R)\iso\hom_\R(\R^2,\R^2)$.
\end{prop}
\begin{proof}
	Let $B=\{b_1,b_2\}$ be a base of $\R^2$. Define $\phi:\hom_\R(\R^2,\R^2)\to M_{2\times 2}(\R)$ by putting
	\[\phi(f):=\begin{pmatrix}
	f(b_1)&f(b_2)
	\end{pmatrix}\]where $f(b_1)$ and $f(b_2)$ are written as $2\times 1$ matrices.
	
	That is, if $f(b_1)=(a_1,a_2)$ and $f(b_2)=(a_3,a_4)$ then we put $\phi(f)=\begin{pmatrix}
	a_1&a_3\\
	a_2&a_4
	\end{pmatrix}$.
	...
	\begin{itemize}
		...
		Assume that $f,g:\R^2\to\R^2$ are two linear functions such that $\phi(f)=\phi(g)$. But then this means that $f(b_1)=g(b_1)$ and $f(b_2)=g(b_2)$. But since linear functions are entirely determined by how they act on a base, this implies that $f=g$, and so $\phi$ is injective.
		
		\item \underline{$\phi$ is surjective:}
		
		Take any $2\times 2$ matrix $A=\begin{pmatrix}
		a&c\\b&d
		\end{pmatrix}$. We then define $f':\R^2\to\R^2$ by putting $f'(b_1):=(a,b)$ and $f'(b_2):=(c,d)$. This defines a unique linear function, by definition of base. Now, clearly, $\phi(f')=A$, so $A\in\im(\phi)$ and $\phi$ is surjective.
	\end{itemize}

This shows that $\phi$ is bijective for each base $B$, and so we have proven the result.
\end{proof}
...
\begin{df}
	Given a base $B\subseteq \R^2$, the bijection $\phi:\hom_\R(\R^2,\R^2)\to M_{2\times 2}(\R)$ induces, for all $X,Y\in M_{2\times 2}(\R)$ and $\lambda\in \R$, $X+Y$, $XY$ and $\lambda X$ by putting $$X+Y:=\phi(f_X+f_Y)$$ $$XY:=\phi(f_X\circ f_Y)$$ $$\lambda X:=\phi(\lambda f_X),$$ where $f_X=\phi^{-1}(X)$ and $f_Y=\phi^{-1}(Y)$.
\end{df}
...
\end{prop}
\begin{proof}
	Let $E=\{e_1,e_2\}$ be the canonical base for $\R^2$. Then $f_X(b_1)=(x_1,x_2)$, $f_X(b_2)=(x_3,x_4)$, $f_y(b_1)=(y_1,y_2)$ and $f_Y(b_2)=(y_3,y_4)$.
	
	It follows then that 
	\[(f_X+f_Y)(e_1)=f_X(e_1)+f_Y(e_1)=(x_1,x_2)+(y_1,y_2)=(x_1+y_1,x_2+y_2)\]
	\[(f_X+f_Y)(e_2)=f_X(e_2)+f_Y(e_2)=(x_3,x_4)+(y_3,y_4)=(x_3+y_3,x_4+y_4)\]
	...
	Similarly, 
	\begin{align*}
		f_X(f_Y(e_1))&=f_X(y_1,y_2)\\
		&=f_X(y_1e_1+y_2e_2)\\
		&=y_1f_X(e_1)+y_2 f_X(e_2)\\
		&=y_1(x_1,x_2)+y_2(x_3,x_4)\\
		&=(x_1y_1,x_2y_1)+(x_3y_2,x_4y_2)=(x_1y_1+x_3y_2,x_2y_1+x_4y_2)		
	\end{align*}
	\begin{align*}
	f_X(f_Y(e_2))&=f_X(y_3,y_4)\\
	&=f_X(y_3e_1+y_4e_2)\\
	&=y_3f_X(e_1)+y_4 f_X(e_2)\\
	...
	Finally,
	\begin{align*}
		(\lambda f_X)(e_1)&=\lambda f_X(e_1)=\lambda (x_1,x_2)=(\lambda x_1,\lambda x_2)
	\end{align*}
	\begin{align*}
	(\lambda f_X)(e_2)&=\lambda f_X(e_2)=\lambda (x_3,x_4)=(\lambda x_3,\lambda x_4)
