...
\begin{lemma}
	Let $X,Y,Z\in M_{2,1}(\R)$ and $\lambda,\mu\in R$. Then:
		\begin{itemize}
			...
			\item There exists a scalar $1\in \R$ such that $1X=X1=X$;
...
\begin{proof}
	\begin{itemize}
		\item \begin{align*}
			...
			&=\psi^{-1}(\psi(\psi^{-1}(\psi_X+\psi_Y))+\psi(Z))\\
			&=\psi^{-1}((\psi_X+\psi_Y)+\psi(Z))\\
			&=\psi^{-1}(\psi_X+(\psi_Y+\psi(Z)))\\
			&=\psi^{-1}(\psi_X+\psi(\psi^{-1}(\psi_Y+\psi(Z))))\\
			&=X+\psi^{-1}(\psi_Y+\psi(Z))=X+(Y+Z)
		\end{align*} so addition is associative;
		
		\item \begin{align*}
			X+T&=\psi^{-1}(\psi_X+\psi_Y)\\
...
\begin{df}
	The evaluation map induces a map $M_{2\times 2}(\R)\times M_{2\times 1}(\R)\to M_{2\times 1}(\R)$ given by 
	\[AX:=f^{-1}()\]
\end{df}