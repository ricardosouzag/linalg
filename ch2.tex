...
\begin{proof}
	...
	\begin{itemize}
		...
		Take, once more, $r=\R(x-y)+x$ the line through both centers. Then, once again, we know this line crosses both circles at points $v,u$, respectively.
		
		Now, since $D>R$, we can write $D=R+\delta$ for some positive $\delta\in \R$. Notice that we can rewrite this as
		\[d_1+d_2=r_2+r_2+\delta\]so
		\[d(x,y)=d(x,v)+d(y,u)+\delta,\]but we also know that
		\[d(x,y)=d(x,v)+d(y,u)+d(v,u),\]since $x,y,v,u$ are colinear. So $0<\delta=d(v,u)$.
	\end{itemize}
\end{proof}