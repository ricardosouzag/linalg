...
\end{cor}
\begin{proof}
	Let $\mc C(x,r)$ be a circle of center $x$ and radius $r\geq0$, $s=\R v+u$ be a line and $y\in\R^2$ be a point such that $d(x,y)<r$ (this is what it means to say that $y$ is in the inside of a circle).
	
	If $y\in s$ then, by definition, $y=\lambda v+u$ for some $\lambda\in \R$. Assume that $d(\lambda' v+u,x)< r$ for all $\lambda_1\in \R$.
\end{proof}
