...
Let now $V^2_A$ be the following set: Take any two perpendicular lines $r$ and $s$ through $A$. Then, $V^2_A$ will be the set $(V_A\cap r)\times (V_A\cap s)$ - that is, the set of all pairs vectors of $E_A$ such that the first vector lies entirely in $r$ and the second vector lies entirely in $s$.

\begin{lemma}
	For any $E_A$, we have that $V_A\iso V_A^2$.
\end{lemma}
\begin{proof}
	Let $f:V_A\to V_A^2$ be the following function: For any vector $\overrightarrow{AP}$ in $V_A$, we can consider the parallel to $r$ through $P$, $r_P$ and the parallel to $s$ through $P$, $s_P$.
	...
	\draw [line width=2.pt,domain=-0.86:8.56] plot(\x,{(--28.1568-2.08*\x)/8.92});
	\draw [line width=2.pt,domain=-0.86:8.56] plot(\x,{(--1.8632--8.92*\x)/2.08});
	\draw [line width=2.pt,dotted,domain=-0.86:8.56] plot(\x,{(-58.4136--8.92*\x)/2.08});
	\draw [line width=2.pt,dotted,domain=-0.86:8.56] plot(\x,{(-51.9936--2.08*\x)/-8.92});
	\begin{scriptsize}
