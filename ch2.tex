...
\begin{ex}
	...
	But $\norm{v}\norm{u}\sin\theta'=\det(v,u)$. So \textbf{the determinant of $v$ and $u$ tells us the are of the parallelogram defined by $v$ and $u$}.
	...
	Notice also that $\det(v,u)=-\det(u,v)$ implies that $\abs{\det(v,u)}=\abs{\det(u,v)}$ so the preceding discussion does indeed make sense: The sign is just telling us who comes first: $v$ or $u$.
\end{ex}