...
\begin{proof}
	...
	Take $u\in I$. Then, as we've shown, $d(u,\mc R)=d(v,\mc R)=d(v',\mc R)$. But we've already proven that the locus of points at a fixed distance from a given line is two lines. This means that either $u\in \mc R c+v$ or $u\in \mc R c+v'$.
	
	In either case there's a point $w'\in \mc R c$ such that $d(u,\mc R c)=d(u,w')$ So if $u\in \mc R c+v$ we get
	\begin{gather*}
		d(u,c)^2=d(u,w')^2+d(w',c)^2\\
		d(v,c)^2=d(v,w)^2+d(w',c)^2\\
		d(w',c)^2=d(v,c)^2-d(v,w)^2\\
		d(w',c)^2=d(w,c)^2
	\end{gather*}so $d(w,c)=d(w',c)$, and since $w,w'\in \mc Rc$ this implies $w'=w$, which, in turn, implies $u=v$ (since $v$ is the point where the line perpendicular to $\mc Rc$ through $w$ meets $\mc Rc+v$, and $u$ is the point what the line perpendicular to $\mc Rc$ through $w'$ meets $\mc Rc+v$).
	
	Doing a similar computation in the case $u\in \mc R c+v'$ shows us that $u=v'$.
