...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Let $0:=f^{-1}(0,0)$. Then $f(0)=(0,0)$, so\[A(X,0)=f^{-1}(f(X)+f(0))=f^{-1}(f(X))=X\]so $0$ is the identity element of $A$;
		
		\item Let $-X:=f^{-1}(-f(X))$. Then \begin{align*}
			f(X)+f(-X)&=f(X)+f(f^{-1}(-f(X)))\\
			&=f(X)-f(X)\\
			&=(0,0)=f(0)
		\end{align*}, so $A(X,-X)=f^{-1}(f(X)+f(-X))=f^{-1}(f(0))=0$ so $-X$ is the additive inverse of $X$;
