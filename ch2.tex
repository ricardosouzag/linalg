...
\begin{proof}
	...
	So we can assume all of these distances are non-zero.
	
	This means that $v-u$ and $v$ are non-zero vectors. 
	
	Assume that $v,u,w$ do not lie in the same line. Therefore, $v-u\nparallel v-w$ and so $V=\{v-u,v-w\}$ is a base for $\R^2$ (since it is a set with two non-parallel vectors). This means that there are some $\lambda_1,\lambda_2\in \R$ such that $v=\lambda_1(v-u)+\lambda_2(v-w)$.
	
	Rearranging this we get $\lambda_1u+\lambda_2w=(\lambda_1+\lambda_2-1)v$.
\end{proof}
