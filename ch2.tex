...
\end{cor}
\begin{proof}
	Let $\mc C:=\mc C(x,r)$ be a circle of center $x$ and radius $r\geq0$, $s=\R v+u$ be a line and $y\in\R^2$ be a point such that $d(x,y)<r$ (this is what it means to say that $y$ is in the inside of a circle).
	
	Assume that $d(y,x)< r$ for all $y\in s$. This means that if we consider the line $s':=\R v$, the point $x':=x-u$ the circle $\mc C':=\mc C(x',r)$, then for all $y'\in s'$ we have
	\begin{align*}
		d(y',x')&=\norm{y'-x'}\\
		&=\norm{\lambda v-(x-u)}\\
		&=\norm{\lambda v+u-x}\\
		&=\norm{y-x}\\
		&=d(y,x)<r
	\end{align*}since $y=\lambda v+u\in s$, and we're assuming that $d(y,x)< r$ for all $y\in s$. This shows that $d(y,x)< r$ for all $y\in s$ if, and only if, $d(y',x')< r$ for all $y'\in s'$.
\end{proof}

\begin{cor}
	Let $x,y\in \R^2$ be any two points and $r_1,r_2\in \R$ be any two non-negative real numbers. Then $\mc C(x,r_1)\cap\mc C(y,r_2)$ is either empty, a single point or two points.
\end{cor}
\begin{proof}
	Let $D:=d(x,y)$ and $R=r_1+r_2$. Let also $I=\mc C(x,r_1)\cap \mc C(y,r_2)$.
	
	Then, we have three cases:
	\begin{itemize}
		\item \underline{$D>R$}:
		
		If there is a point $v\in I$, then $vxy$ is a triangle, so 
		\[d(x,y)\leq d(x,v)+d(y,v).\]
		
		But $v\in I$ implies $d(v,x)=r_1$ and $d(v,y)=r_2$, so $d(x,v)+d(y,v)=R$. Then the above inequality becomes $D\leq R$, which contradicts our assumption that $D>R$.
		
		Therefore, if $D>R$ then $I=\varnothing$.
		
		\item \underline{$D=R$}:
		
		Take the line through both centers, $r:=\R(x-y)+x$. Then, the preceding lemma tells us that this line meets both circles. Call these points $v,u$, resp.
		
		Now, for each of those points we have that $d(v,x)+d(v,y)=D=d(u,x)+d(u,y)$, or, in other words, since $v\in \mc C(x,r_1)$ and $u\in \mc C(y,r_2)$, this is
		\[r_1+d(v,y)=D=d(u,x)+r_2\]but $D=R$ implies
		\[r_1+d(v,y)=r_1+r_2=d(u,x)+r_2\]and so the LHS yields $d(v,y)=r_2$, and the RHS yields $d(u,x)=r_1$. This shows that $v\in \mc C(y,r_2)$ and $u\in \mc C(x,r_1)$.
		
		So $v$ and $u$ lie, at once, both in $r$ and in both circles.
		
		But then, 
		\[d(v,x)= d(v,u)+d(u,x)\]tells us that $d(v,u)=0$, so $v=u$ and we see that $I$ is a single point.
