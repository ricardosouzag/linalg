...
\begin{proof}
	...
	\begin{itemize}
		...
		Assume that $A,B\in M_{2\times 2}(\R)$ are two matrices such that $\tau_A=\tau_B$. This means, in particular, that $\tau_A(e_1)=\tau_B(e_1)$ and $\tau_A(e_2)=\tau_B(e_2)$. But this implies that the first and second columns of $A$ and $B$ are equal. Since they only have two columns, this implies $A=B$, and so $\varphi$ is injective.
