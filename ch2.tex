...
\begin{proof}
	...
	Let $r=\R v$ be a line through zero, such that $u,w\in r$. Then $u=\mu v$ and $w=\omega v$ for some $\mu,\omega\in \R$. Notice that we can assume that all three points are different, cause, otherwise, the result is trivially true. This implies $\mu\neq \omega$ and both different from $1$.
	...
	\begin{align*}
		...
		&=\norm{v-\mu v}\\
		&=\norm{(1-\mu)v}=\abs{1-\mu}\norm{v}
	\end{align*}and similarly $d(v,w)=\abs{1-\omega}\norm{v}$ and $d(u,w)=\abs{\mu-\omega}\norm{v}$.
	
	Therefore, if $w$ is between $v$ and $u$, it means that $1<\omega<\mu$, so
	\begin{align*}
		d(v,u)&=\abs{1-\mu}\norm{v}\\
		&=(\mu-1)\norm{v}\\
		&=(\mu-\omega+\omega-1)\norm{v}\\
	...
	\bigskip
	Conversely, assume $v,u,w\in \R^2$ are such that the equality holds, that is, 
	\[d(v,u)=d(v,w)+d(u,w).\]
	
	Let us call $d_1=d(v,u)$, $d_2=d(v,w)$ and $d_3=d(u,w)$.
	
	Then if either of these is zero, the result follows - for instance, $d_2=0$ implies $v=w$ (so they lie in the same line), and since between any two points there's always a line, this implies that $u$ is also in a line with $v$ and $w$.
	
	So we can assume all of these are non-zero.
	
	This means that $v-u,v-w$ and $u-w$ are non-zero vectors. 
	
	Assume that $v,u,w$ do not lie in the same line. Therefore, $v-u\nparallel v-w$ and so $V=\{v-u,v-w\}$ is a base for $\R^2$ (since it is a set with two non-parallel vectors).
\end{proof}
