...
\begin{lemma}
	Let $v_1v_2\cdots v_n$ be an $n$-gon in $\R^2$. Then its (unsigned) area is just
	\[\frac{\abs{\det(v_1,v_2)+\det(v_2,v_3)+\cdots+\det(v_{n-1},v_n)}}{2}.\]
\end{lemma}
\begin{proof}
	...
	$$A(v,u,w)=\frac{\abs{\det(v,u)+\det(u,w)+\det(w,v)}}{2}$$ for any $v,u,w\in \R^2$. So $A(v_1,v_i,v_{i+1})$ becomes
	\[\frac{\abs{(\det(v_1,v_i)+\det(v_i,v_{i+1})+\det(v_{i+1},v_1)}}{2}\]for all $2\leq i\leq n-1$.
\end{proof}