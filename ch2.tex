...
\begin{df}
	A set $X\subseteq \R^2$ such that any function $f:X\to \R^2$ extends uniquely to a linear function will be called a \textbf{base} or a \textbf{basic set} for $\R^2$.
\end{df}
...
\begin{ex}
	...
	What this tells us is that lines are \textbf{closed} under addition and scaling - that is, if we add two points on the same line, then they remain on that line, and if we scale any point on a line, it also remains there.
\end{ex}
...
\begin{ex}
	...
	Now this shows us immediately that the lines $\R v$ and $\R u$ are the same line: Take any $w\in \R v$. That means that $w=\omega(1,1)=(\omega,\omega)$ for some $\omega\in \R$.
	 ...
	 But now, taking $\omega':=\frac{-3}{\omega}$, we see that 
	 \[\omega' w=\left(\frac{-3}{\omega}\omega\right)v=-3v=u\]and so $w=\omega'^{-1}u$, and we see that $w\in \R u$, just as stated.
\end{ex}