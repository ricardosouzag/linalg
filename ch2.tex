...
\begin{proof}
	...
	\begin{itemize}
		...
		Now, for each of those points we have that $d(v,0)+d(v,c)=D=d(u,0)+d(u,c)$, or, in other words, since $v\in \mc C(0,r_1)$ and $u\in \mc C(c,r_2)$, this is
		\[r_1+d(v,c)=D=d(u,0)+r_2\]but $D=R$ implies
		\[r_1+d(v,c)=r_1+r_2=d(u,0)+r_2\]and so the LHS yields $d(v,c)=r_2$, and the RHS yields $d(u,0)=r_1$. This shows that $v\in \mc C(c,r_2)$ and $u\in \mc C(0,r_1)$.
