...
\begin{cor}
	Every line in $\R^2$ is just the subset of $\R^2$ formed by all scalar multiples of a fixed vector.
\end{cor}
...
\begin{proof}
	...
	\begin{enumerate}[(1)]
		\item $$(\mu\lambda)v=(\mu\lambda)(v_1,v_2)=((\mu\lambda)v_1,(\mu\lambda)v_2)=(\mu(\lambda v_1),\mu(\lambda v_2))=\mu(\lambda v_1,\lambda v_2)=\mu(\lambda v)$$
	\end{enumerate}
\end{proof}