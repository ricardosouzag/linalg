...
\begin{df}
	The bijection $\phi:\hom_\R(\R^2,\R^2)\to M_{2\times 2}(\R)$ induces, for all $X,Y\in M_{2\times 2}(\R)$ and $\lambda\in \R$, $X+Y$, $XY$ and $\lambda X$ by putting $$X+Y:=\phi(f_X+f_Y)$$ $$XY:=\phi(f_X\circ f_Y)$$ $$\lambda X:=\phi(\lambda f_X),$$ where $f_X=\phi^{-1}(X)$ and $f_X=\phi^{-1}(Y)$.
\end{df}