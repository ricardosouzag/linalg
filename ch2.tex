...
\begin{proof}
	...
	\begin{itemize}
		...
		Take the line through both centers, $r:=\R c$. Then, one of the preceding lemmas tells us that this line meets both circles. Call these points $v,u$, resp.
		
		Now, for each of those points we have that $d(v,0)+d(v,c)=D=d(u,0)+d(u,c)$, or, in other words, since $v\in \mc C_1$ and $u\in \mc C_2$, this is
		\[r_1+d(v,c)=D=d(u,0)+r_2\]but $D=R$ implies
		\[r_1+d(v,c)=r_1+r_2=d(u,0)+r_2\]and so the left equality yields $d(v,c)=r_2$, and the right equality yields $d(u,0)=r_1$. This shows that $v\in \mc C_2$ and $u\in \mc C_1$.
		
		So $v$ and $u$ lie, at once, both in $r$ and in $I$.
		
		But then, since they are colinear either or both of
		\[d(u,0)= d(v,u)+d(v,0)\]and
		\[d(v,0)= d(v,u)+d(u,0)\] hold (that is, either $u$ is between $v$ and $0$ or $v$ is between $u$ and $0$.)
		
		It really doesn't matter which one holds, because they both tell us that $d(v,u)=0$, so $v=u$ and we see that $I$ is a single point.
		
		\item \underline{$D>R$}:
		
		Take, once more, $r=\R c$ the line through both centers. Then, once again, we know this line crosses both circles at points $v,u$, respectively.
		...
		\[d(0,c)=d(0,v)+d(c,u)+\delta,\]but we also know that
		\[d(0,c)=d(0,v)+d(c,u)+d(v,u),\]since $0,c,v,u$ are colinear. So $\delta=d(v,u)$.
	\end{itemize}
\end{proof}