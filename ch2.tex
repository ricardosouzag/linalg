...
\end{ex}
\begin{prop}
	The inner product preserves scalar multiplication - that is, for any two vectors $v,u\in \R^2$ and $\lambda\in \R$ we have $\gen{v,\lambda u}=\lambda\gen{v,u}$.
\end{prop}
\begin{proof}
	Let, once again, $v=(v_1,v_2)$ and $u=(u_1,u_2)$. Then $\lambda u=(\lambda u_1,\lambda u_2)$ and so:
	\[\gen{v,\lambda u}=v_1(\lambda u_1)+v_2(\lambda u_2)=\lambda(v_1u_1)+\lambda (v_2u_2)=\lambda(v_1u_1+v_2u_2)=\lambda\gen{v,u}\]so the inner product preserves scalar multiplication, which proves the result.
\end{proof}
...
\end{cor}
\begin{proof}
	Remember, from geometry, that if $ABC$ is a triangle with sides $a,b,c\in\R$ and angle $\theta\in[0,2\pi)$ between $a$ and $b$, then the Cosine Law tells us that
