...
\begin{lemma}
	Let $X,Y,Z\in M_{2,1}(\R)$ and $\lambda,\mu\in R$. Then:
		\begin{itemize}
			\item $(X+Y)+Z=X+(Y+Z)$
			\item $X+Y=Y+X$;
...
\end{prop}
\begin{proof}
	Let $E=\{e_1,e_2\}$ be the canonical base of $\R^2$. Define $\phi:\hom_\R(\R^2,\R^2)\to M_{2\times 2}(\R)$ by putting
	\[\phi(f):=\begin{pmatrix}
	f(e_1)&f(e_2)
	\end{pmatrix}\]where $f(e_1)$ and $f(e_2)$ are written as $2\times 1$ matrices.
	
	That is, if $f(e_1)=(a_1,a_2)$ and $f(e_2)=(a_3,a_4)$ then we put $\phi(f)=\begin{pmatrix}
	a_1&a_3\\
	a_2&a_4
	\end{pmatrix}$.
	...
	\begin{itemize}
		...
		Assume that $f,g:\R^2\to\R^2$ are two linear functions such that $\phi(f)=\phi(g)$. But then this means that $f(e_1)=g(e_1)$ and $f(e_2)=g(e_2)$. But since linear functions are entirely determined by how they act on a base, this implies that $f=g$, and so $\phi$ is injective.
		
		\item \underline{$\phi$ is surjective:}
		
		Take any $2\times 2$ matrix $A=\begin{pmatrix}
		a&c\\b&d
		\end{pmatrix}$. We then define $f':\R^2\to\R^2$ by putting $f'(e_1):=(a,b)$ and $f'(e_2):=(c,d)$. This defines a unique linear function, by definition of base. Now, clearly, $\phi(f')=A$, so $A\in\im(\phi)$ and $\phi$ is surjective.
	\end{itemize}
...
\begin{df}
	The bijection $\phi:\hom_\R(\R^2,\R^2)\to M_{2\times 2}(\R)$ induces, for all $X,Y\in M_{2\times 2}(\R)$ and $\lambda\in \R$, $X+Y$, $XY$ and $\lambda X$ by putting $$X+Y:=\phi(f_X+f_Y)$$ $$XY:=\phi(f_X\circ f_Y)$$ $$\lambda X:=\phi(\lambda f_X),$$ where $f_X=\phi^{-1}(X)$ and $f_Y=\phi^{-1}(Y)$.
\end{df}
...
\end{lemma}
\begin{proof}
	Let, once more, $E=\{e_1,e_2\}$ be the canonical base of $\R^2$. Then $f_A(e_1)=(a_1,a_2)$ and $f_A(e_2)=(a_3,a_4)$.
	
	Therefore,
	\begin{align*}
		f_A(\psi_X)&=f_A(x_1,x_2)\\
		&=f_A(x_1e_1+x_2e_2)\\
		&=x_1f_A(e_1)+x_2f_A(e_2)\\
...
To finally and this section, we'll now introduce the $1\times 2$ matrices.
...
\begin{prop}
	The function $\tau:\hom_\R(\R^2,\R^2)\to M_{2\times 2}(\R)$ given by $$\tau(f):=\begin{pmatrix}
	f(e_1)\\f(e_2)
	\end{pmatrix}$$where $f(e_1)$ and $f(e_2)$ are written as $1\times 2$ matrices is a bijection.
\end{prop}
\begin{proof}
	Just to make it clear, we're defining $$\tau(f)=\begin{pmatrix}
	a_1&a_2\\a_3&a_4
	\end{pmatrix}$$where $f(e_1)=(a_1,a_2)$ and $f(e_2)=(a_3,a_4)$ as usual.
	
	\begin{itemize}
		\item \underline{$\tau$ is injective:}
		
		Assume that $f,g:\R^2\to\R^2$ are two linear functions such that $\tau(f)=\tau(g)$. But then this means that $f(e_1)=g(e_1)$ and $f(e_2)=g(e_2)$. But since linear functions are entirely determined by how they act on a base, this implies that $f=g$ so $\tau$ is injective.
		
		\item \underline{$\tau$ is surjective:}
		
		Take any $2\times 2$ matrix $A=\begin{pmatrix}
		a&b\\c&d
		\end{pmatrix}$. We then define $f':\R^2\to\R^2$ by putting $f'(e_1):=(a,b)$ and $f'(e_2)=(c,d)$. This defines a unique linear function, by definition of base. Now, clearly, $\tau(f')=A$, so $A\in\im(\tau)$ and $\tau$ is surjective.
	\end{itemize}
