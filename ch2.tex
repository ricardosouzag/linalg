...
\begin{ex}
	Consider the lines $X$ (that is, $\R e_1$) and $Y$ (that is, $\R e_2$). Then $X\cup Y$ is just the two axis together.
	
	It clearly is closed under scalar multiplication, but, sadly, it is not closed under addition. This is very easy to see:
	
	For instance, $e_1\in X$ and $e_2\in Y$, so $e_1,e_2\in X\cup Y$, by definition of set union.
	
	If $X\cup Y$ was a subspace, then $e_1+e_2\in X\cup Y$, but $e_1+e_2=(1,1)$ and we know that $(1,1)\notin X$  and $(1,1)\notin Y$ and, therefore, $(1,1)\notin X\cup Y$.
...
\begin{ex}
	...
	Take the line $X$ and the line $Y$, once again. What is $X+Y$? Well, by what we just did, it's the set of all sums of points in $X$ and $Y$...
	
	But this is the whole $\R^2$!
	
	To see this, take any $v\in \R^2$. Then $v=(x,y)$, for some $x,y\in \R$, and we can rewrite this as $xe_1+ye_2$. But since $X$ and $Y$ are subspaces, $xe_1\in X$ and $ye_2\in Y$, so $v\in X+Y$.
	
	This shows that $X+Y\subseteq \R^2\subseteq \R^2$. That can only mean one thing: $X+Y=\R^2$.
\end{ex}
...
\begin{proof}
	...
	We'll show that $r+s$ contains both $e_1$ and $e_2$. This suffices, because we've already shown that $X+Y=\R^2$.
	...
	\[v-\mu_1u=(v_1,v_2)-\left(\frac{v_2}{u_2}u_1,v_2\right)=\left(v_1-\frac{v_2}{u_2}u_1,0\right),\]that is, $v-\mu_1u\in X$. So we can just scale it up or down to obtain $e_1$. To do that, let us rewrite it a little bit:
	...
	Now we can just argue: Since $v\in r$ and $\mu_1u\in s$, we get that $v-\mu_1u\in r+s$. But since $r+s$ is a subspace, we see that $\mu_2(v-\mu_1u)\in r+s$, which shows that $e_1$ (and all its scalar multiples) is also in $r+s$.
	...
	Now, since $\lambda_1v\in r$ and $u\in s$, we see that $\lambda_1v-u\in r+s$, and so, since it is a subspace, $\lambda_2(\lambda_1v-u)\in r+s$. This shows that $e_2$ (and all its scalar multiples) is also in $r+s$.
	
	\bigskip
	Finally, this shows that $r+s$ contains both of the $X$- and the $Y$-axis, and, therefore, by what we did in the preceding example, it must be the whole plane.
