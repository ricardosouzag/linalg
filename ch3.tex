...
\end{df}
\begin{df}
	A finite set $X\subseteq\R^3$ is called a \textbf{base} of $\R^3$ if any linear function is uniquely determined by the image of $X$ - that is, if we write $X=\{x_1,x_2,\cdots,x_n\}$, then for all $v\in \R^3$, there are uniquely determined $\lambda_1,\cdots,\lambda_n\in \R$ such that $v=\lambda_1x_1+\cdots+\lambda_nx_n$ and thus the image of $v$ under any linear function $f:\R^3\to\R^3$ is given simply by $f(v)=\lambda_1f(x_1)+\cdots+\lambda_nf(x_n)$.
\end{df}
\begin{df}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\bigskip
		Finally, let $f:\R^3\to\R^3$ be any linear function, and let $v=\sum_{i=1}^3v_ix_i$ as before (but now we know that the $v_i$s are uniquely determined). We can now write $f(v)$ as
		
		\[f(v)=\sum_{i=1}^3v_if(x_i),\]by simply applying $f$ on $v$ and using the fact that $f$ is linear. This shows that $f$ is entirely determined by $X$, so $X$ is a base.
	\end{itemize}
