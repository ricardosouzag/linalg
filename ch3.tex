...
\begin{ex}
	Just as an example of all of these results, let's compute the determinant of the canonical basis vectors: $e_1,e_2,e_3$. To do that, we first need to compute $e_2\times e_3$:
	...
	Notice that we could have accomplished this by simply using the above lemma:
...
\end{ex}
\begin{cor}
	Let $v,u,w\in \R^3$ be non-coplanar vectors. Then $\det(v,u,w)>0$ if, and only if, either of $w\leq u\leq v$, $u\leq v\leq w$ or $v\leq w\leq u$ holds.
\end{cor}
\begin{proof}
	From one of the previous results, we already know that $\det(v,u,w)=\lambda_3\norm{u\times w}^2$, where $v=\lambda_1u+\lambda_2w+\lambda_3(u\times w)$. From this we know that the sign of $\det(v,u,w)$ is just the sign of $\lambda_3$.
	
	Now look at $u$ and $w$. Since $\R^3$ is totally ordered, then either $u\geq w$ or $u\leq w$.
