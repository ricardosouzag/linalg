...
\begin{lemma}
	Let $X,Y,Z\leq \R^3$. Then $X+(Y\cap Z)=(X+Y)\cap(X+Z)$ and $X\cap(Y+Z)=(X\cap Y)+(X\cap Z)$.
\end{lemma}
\begin{proof}
	...
	Conversely, take $v\in(X+Y)\cap(X+Z)$. However, $X\subseteq X+Y$ and $X\subseteq X+Z$ implies, by definition of intersection, that $(X+Y)\cap(X+Z)\subseteq X$ - so $v\in X$. Finally, since $X\subseteq X+(Y\cap Z)$, this shows that $v\in X+(Y\cap Z)$, so
...
\begin{proof}
	...
	So clearly, $t'\in \R v+\R u$ implies both of $t'\in \R t+\R u$ and $t'\in \R v+\R t$ - and therefore, $t'\in (\R v+\R t)\cap(\R t+\R u)$.
	...
	\bigskip
	Now, finally, take $x\in \R t+\R u$ as above. Then:
	\begin{align*}
		x&=\tau t+\mu u\\
		&=\tau+\mu(t-\tau_1v)
	\end{align*}
