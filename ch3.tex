...
\begin{proof}
	...
	Now all that's left is to show that it is a total order - i.e., that given any two vectors $v,u\in\R^3$ then either $v\leq u$ or $u\leq v$ hold.
	
	To do that, take any $v,u\in \R^3$. If $v\leq u$, then we're done. Otherwise, by definition of $\leq$, we have that all of $v(1)u$, $v(2)u$, $v(3)u$ and $v(4)u$ fail (because, once again by definition of $\leq$, if at least one of them was true, then we'd have $v\leq u$).
	
	\begin{itemize}
		\item $v(1)u$ failing tells us that $v_1\not<u_1$ - or, in other words, $v_1\geq u_1$, since $\R$ is totally ordered.
		\item $v(2)u$ failing tells us that either or both of these is true: $v_1\neq u_1$ and $v_2\not<u_2$. But since $\R$ is totally ordered, $v_2\not<u_2$ is just the same as $v_2\geq u_2$.
		\item $v(3)u$ failing tells us that either or all of these is true: $v_1\neq u_1$, $v_2\neq u_2$ and $v_3\not<u_3$. Once again, since $\R$ is totally ordered, $v_3\not<u_3$ is the same as $v_3\geq u_3$.
		\item $v(4)u$ failing tells us that either or all of these is true: $v_1\neq u_1$, $v_2\neq u_2$ and $v_3\neq u_3$.
	\end{itemize}	
\end{proof}
We can start off by giving an intuitive definition:
\begin{df}
	Let $f:\R^3\to\R^3$ be a linear function. We define the \textbf{determinant of $f$} to be the real number $\det f$ given by the signed volume of the cube with edges $f(e_1),f(e_2),f(e_3)$. In other words
\end{df}