	...
	\begin{itemize}
		...
		Let $v,u,w\in \R^3$ be such that $v\leq u$ and $u\leq w$. We need to show that this implies $v\leq w$. We once again need to do that by considering all possible cases. To do so, we'll use the following table:
			\begin{tabu}{|c|[2pt]c|c|c|c|}
				\hline
				$\leq$ & $u(1)w$ & \(u(2)w\) & \(u(3)w\) & \(u(4)w\)\\\tabucline[2pt]{-}
				$v(1)u$ & \\\hline
				$v(2)u$ & $v(1)w$ & $v(2)w$ & $v(2)w$ & $v(2)w$\\\hline
				$v(3)u$ & $v(1)w$ & $v(2)w$ & $v(3)w$ & $v(3)w$\\\hline
				$v(4)u$ & $v(1)w$ & $v(2)w$ & $v(3)w$ & $v(4)w$\\\hline
			\end{tabu}Here, $u(2)w$, for instance, should be read as ``$u\leq w$ because they satisfy condition (2) in the definition of $\leq$'' - or, in other words, $u(2)w$ means ``$u_1=w_1$, but $u_2<w_2$'' - and so on.
		
		That said, this is how this table should be read: For instance, suppose we have $v(3)u$ and $u(2)w$. This means that ``$v_1=u_1$ and $v_2=u_2$, but $v_3<u_3$'' and ``$u_1=w_1$, but $u_2<w_2$''.
