...
\begin{df}
	Let $(X,\leq)$ be a totally ordered set. A function $f:X\to X$ is said to \textbf{preserve cyclic order} if for all $x\leq y\leq z$ in $X$ we have that either of $f(x)\leq f(y)\leq f(z)$, $f(y)\leq f(z)\leq f(x)$ or $f(z)\leq f(x)\leq f(y)$ hold.
\end{df}
