...
\end{prop}
\begin{proof}
	If $f$ is a cycle, then either $w\leq u\leq v$, $u\leq v\leq w$ or $v\leq w \leq u$.
	
	Define $f',f'':\R^3\to\R^3$ linear functions by putting $v=f(e_1)=f'(e_2)=f''(e_3)$, $u=f(e_2)=f'(e_3)=f''(e_1)$ and $w=f(e_3)=f'(e_2)=f''(e_1)$.
	
	Now we have that $\det f=\det f'=\det f''$, since $\det(v,u,w)=\det(u,w,v)=\det(w,v,u)$.
\end{proof}
