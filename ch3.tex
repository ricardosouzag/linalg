...
\begin{proof}
	\begin{enumerate}[a)]
		\item The previous theorem tells us that $\norm{\det(v,u,w)}=\norm{\det(u,w,v)}=\norm{\det(w,v,u)}$, since they all describe the same real number: The volume of a parallelepiped with sides $v,u,w$. So all that's left for us to do is prove that they have the same sign.
	\end{enumerate}
\end{proof}