...
\newpage
\section{Back to linearity in $\R^3$}
...
\begin{proof}
	...
	\begin{itemize}
		...
		Suppose $f(x_1)\parallel f(x_2)$ - that is, $f(x_1)=\lambda f(x_2)$ for some $\lambda\in \R$. But, since $X$ is a base, we have that there are uniquely determined $a_1,a_2,a_3,b_1,b_2,b_3\in \R$ such that
		\[f(x_1)=\sum_{i=1}^na_ix_1\]
		\[f(x_2)=\sum_{i=1}^nb_ix_i.\]
		
		Combining these two with our assumption that $f(x_1)=\lambda f(x_2)$ we get
		\[\sum_{i=1}^n a_ix_i=\sum_{i=1}^n (\lambda b_i)x_i\]and since $X$ is a base, we see that $a_i=\lambda b_i$ for all $i\leq 3$.
	\end{itemize}
\end{proof}