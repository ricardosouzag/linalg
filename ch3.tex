...
\begin{proof}
	...
	But since $u$ is non-null, we cannot have $u_2=0$. Therefore, we can write
...
\begin{proof}
	...
	Let $v=(v_1,v_2,v_3)$, $u=(u_1,u_2,u_3)$, $p=(p_1,p_2,p_3)$ and $q=(q_1,q_2,q_3)$.
	
	But $v,u\in\pi$ implies the existence of $\lambda_1,\lambda_2,\mu_1,\mu_2\in \R$ such that
	...
	\[u-v'=(\mu_1p+\mu_2q)-\mu_1p=\mu_2q.\] Now, since both $v$ and $u$ are non-null, this implies that $\lambda_1$ and $\mu_2$ cannot be zero. So we can write
	\[p=\frac{1}{\lambda_1}v\in\R v\]
	\[q=\frac{1}{\mu_2}(u-v')\in\R v+\R u,\]so $\R v+\R u$ contains both $p$ and $q$ - and thus it contains $\pi$.
\end{proof}

\begin{prop}
	Let $v,u,w\in\R^3$ non-null non-coplanar vectors. Then $\R v+\R u+\R w=\R^3$.
\end{prop}
\begin{proof}
	This proof consists on showing that $e_1,e_2,e_3\in \R v+\R u+\R w$. This suffices, by what we did above.
	
	To start things off, we write, as usual, $v=(v_1,v_2,v_3)$, $u=(u_1,u_2,u_3)$ and $w=(w_1,w_2,w_3)$. For now, we assume all of these coordinates are non-zero. We'll deal with the case when they're zero later:
	
	First, take $u':=\dfrac{v_3}{u_3}u\in \R u$. Then: $$u'=\dfrac{v_3}{u_3}u=\left(\dfrac{v_3}{u_3}u_1,\frac{v_3}{u_3}u_2,v_3\right).$$Just for simplicity, let's write $u'_1:=\dfrac{v_3}{u_3}u_1$ and $u'_2:=\dfrac{v_3}{u_3}u_2$. Then:
	\[v-u'=(v_1-u'_1,v_2-u_2',0)\in \mb{XY}.\]
