...
\end{proof}
\begin{cor}
	Let $f:\R^3\to\R^3$ be a linear function, and let $g,g',g'':\R^3\to \R^3$ be linear functions such that
	\[f(e_1)=g(e_1)=g'(e_2)=g''(e_3)\]
	\[f(e_2)=g(e_3)=g'(e_1)=g''(e_2)\]
	\[f(e_3)=g(e_2)=g'(e_3)=g''(e_1)\] (in other words, the matrices of $g$, $g'$ and $g''$ are just the matrix of $f$, but with two columns swapped). Then $\det g=\det g'=\det g''=-\det f$ as well as $\det A_g=\det A_{g'}=\det A_{g''}=-\det A_f$.
\end{cor}
\begin{proof}
	Once again, this follows from the fact that $\det(u,v,w)=\det(w,u,v)=\det(v,w,u)=-\det(v,u,w)$ for any three vectors $v,u,w\in \R^3$.
\end{proof}