...
\begin{prop}
	Let $f:\R^3\to\R^3$ be a linear function and $v,u,w\in\R^3$ be any three non-coplanar vectors. Then $\det(f(v),f(u),f(w))=\det f\cdot \det (v,u,w)$.
\end{prop}

\begin{prop}
	Let $f:\R^3\to \R^3$ be a linear function and let $v=f(e_1)$, $u=f(e_2)$ and $w=f(e_3)$. Then $\det(v,u,w)$ is positive if $f$ is a cycle, and negative otherwise.
\end{prop}
\begin{proof}
	Define $f',f'':\R^3\to\R^3$ linear functions by putting $$v=f(e_1)=f'(e_2)=f''(e_3)$$ $$u=f(e_2)=f'(e_3)=f''(e_1)$$ $$w=f(e_3)=f'(e_1)=f''(e_2).$$
	
	Now we have that $\det f=\det f'=\det f''$, since $\det(v,u,w)=\det(w,v,u)=\det(u,w,v)$.
	
	If $f$ is a cycle, then either $f$, $f'$ or $f''$ preserve the order $e_3\leq e_2\leq e_1$. Indeed, if $f$ is a cycle then either $w\leq u\leq v$, $u\leq v\leq w$ or $v\leq w\leq u$. In the first case $f$ preserves the order (since $f(e_3)\leq f(e_2)\leq f(e_1)$), in the second case $f'$ preserves the order (since $f'(e_3)\leq f'(e_2)\leq f'(e_1)$) and in the third case $f''$ preserves the order (since $f''(e_3)\leq f''(e_2)\leq f''(e_1)$).
\end{proof}

Finally we can prove the theorem.

\begin{proof}
	Let $f:\R^3\to\R^3$ be any linear function, and let, as usual, $e_1,e_2,e_3$ be the canonical base. Denote $v=f(e_1),u=f(e_2),w=f(e_3)$.
	
	We already know that $\abs{\det(v,u,w)}=\abs{\det f}$ since they both describe the volume of the parallelepiped with sides $v,u,w$.
	
	We now ask: Do they have the same sign? If they do, then we're done. Otherwise, define $g:\R^3\to \R^3$ by putting $g(e_1)=v,g(e_2)=w,g(e_3)=u$.
	
	Now, clearly either $g$ or $f$ is cyclic: If $f$ is cyclic, then either $w\leq u\leq v$, $u\leq v\leq w$ or $v\leq w\leq u$ hold (since $e_3\leq e_2\leq e_1$). But no matter which one of these hold, $g$ can't be cyclic: If the first one holds, then $g(e_2)\leq g(e_3)\leq g(e_1)$; if the second one holds, then $g(e_3)\leq g(e_1)\leq g(e_2)$; if the third one holds, then $g(e_1)\leq g(e_2)\leq g(e_3)$.
	
	Analogously we can show, by the same argument, that if $g$ is cyclic, then $f$ cannot be.
	
	This shows that $\det f=-\det g$. We also already know that $\det(v,u,w)=\det(v,w,u)$.
	
	Combining these two, we see that either $\det f=\det(v,u,w)$ or $\det g=\det (v,u,w)$.
\end{proof}