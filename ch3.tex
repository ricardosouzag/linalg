...
\begin{proof}
	...
	Conversely, if $v\in \R u+\R w$ then $v$ is orthogonal to any vector $p$ such that $p^\perp=\R u+\R w$. Since $(u\times w)=\R u+\R w$, by definition, we have that $v\perp u\times w$ and thus $\gen{v,u\times w}=0$. But $\gen{v,u\times w}=\det(v,u,w)$, so the result follows.
\end{proof}
