...
\end{lemma}
\begin{proof}
	Since $v\parallel u$, $\R v=\R u$, so, clearly, $u\in \R v+\R w$ and thus $\plane{v}{u}{w}$.
\end{proof}
...
\begin{proof}
	...
	Now, if any two of the last coordinates above are zero - say, $v_3=u_3=0$ - then clearly $\R v+\R u$ already contains $\mb{XY}$ (since $v,u,w$ are coplanar and, therefore, $v,u$ are non-parallel).
