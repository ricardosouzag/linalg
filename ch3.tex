...
\begin{prop}
	Let $v,u,w\in\R^3$ non-coplanar vectors. Then $\R v+\R u+\R w=\R^3$.
\end{prop}
\begin{proof}
	...
	First, take $$u'=\dfrac{v_3}{u_3}u=\left(\dfrac{v_3}{u_3}u_1,\frac{v_3}{u_3}u_2,v_3\right)$$
\end{proof}
...
\end{prop}
\begin{proof}
	Let $\pi=\R v+\R u$ and $\sigma=\R w+\R z$ for some vectors $v,u,w,z\in \R^3$.
	
	We will show that $\pi+\sigma$ contains $\mb{XY+YZ}$. This suffices, by the preceding lemma.
	
	Let $v=(v_1,v_2,v_3), u=(u_1,u_2,u_3), w=(w_1,w_2,w_3)$ and $z=(z_1,z_2,z_3)$.
	
	If $v_3=0$ this means that $v\in\mb{XY}$ so $\pi+\sigma$ contains $\mb{XY}$. Similarly, if $u_1=0$ this means that $u\in\mb{YZ}$, so $\pi+\sigma$ contains $\mb{YZ}$.
	
	Now we define
	\[\omega=\frac{w_3}{v_3},\quad\quad\zeta=\frac{z_1}{u_1}.\]
	
	Then:
	\begin{align*}
		 &=\omega(w_1,w_2,w_3)\\
		&=(\omega w_1,\omega w_2,v_3)
	\end{align*}so
	\[v-\omega w=(v_1-\omega w_1,v_2-\omega w_2,0)\in\mb{XY}\]and similarly,
	\begin{align*}
		\zeta z&=\zeta(z_1,z_2,z_3)\\
		&=(u_1,\zeta z_2,\zeta z_3)
	\end{align*}so
	\[u-\zeta z=(0,\zeta z_2,\zeta z_3)\in\mb{YZ}.\]
\end{proof}