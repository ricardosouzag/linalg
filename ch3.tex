...
\end{cor}
\begin{proof}
	Let $x\in\R^3$ be any vector. We want to show that $x\in \R v+\R u+\R w$.
	
	If $x$ is in either $\R v$, $\R u$ or $\R w$ the result already follows.
	
	Similarly, if $x$ is in either $\R v+\R u$, $\R u+\R w$ or $\R w+\R v$, the result already follows.
	
	All that's left then is to consider the case where $x$ is in neither $\R v$, $\R u$, $\R w$, $\R v+\R u$, $\R u+\R w$ or $\R w+\R v$.
\end{proof}