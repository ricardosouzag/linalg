...
\begin{cor}
	Let $X,Y\leq \R^3$ be two subspaces of $\R^3$. Then:
	\begin{enumerate}[a)]
		\item If $X=Y=0$, then $X+Y=X\cap Y=0$;
		\item If $X$ and $Y$ are lines, then $X+Y$ is a plane and $X\cap Y=0$ if, and only if, $X\nparallel Y$;
		\item If $X$ and $Y$ are planes, then $X+Y=\R^3$ and $X\cap Y$ is a line if, and only if, $X\nparallel Y$.
	\end{enumerate}
\end{cor}
\begin{proof}
	\begin{enumerate}[a)]
		...
		Assume now that $X\nparallel Y$ - that is, $X\neq Y$ (since they're both through zero). 
		
		But then, this tells us that any three of $\{v,u,p,q\}$ are non-coplanar - and hence that the sum of the lines through them is $\R^3$.
		
		For instance, $v,u,p$ are non-coplanar, so $\R v+\R u+\R p=\R^3$. But this means that there exist some $q_1,q_2,q_3\in\R$ such that 
		\[q=q_1v+q_2u+q_3p.\]
		
		But we can rearrange this into
		\[-q_3p+q=q_1v+q_2v\]which tells us that $-q_3p+q\in \R v+\R u=X$, but since $-q_3p+q\in\R p+\R q= Y$, by definition, so we have that $-q_3p+q\in X\cap Y$ and therefore, $\R (-q_3p+q)\subseteq X\cap Y$.
		
		So we know that $X\cap Y$ contains a line and isn't a plane - so it can only be that line.
		
		\bigskip
		Finally, if $X\cap Y$ is a line, then let $\R z=X\cap Y$. This means that $z=x_1v+x_2u=y_1p+y_2q$ for some $x_1,x_2,y_1,y_2\in\R$. But since $\R z$ is a line, we get that $z\neq 0$, and so at least one of $x_1,x_2,y_1$ and $y_2$ must be non-zero.
		
		Assume, without loss of generality, that $x_1\neq 0$. This means that we can rearrange the previous equation:
		\begin{gather*}
			x_1v+x_2u=y_1p+y_2q\\
			x_1v=y_1p+y_2q-x_2u\\
			v=\frac{y_1}{x_1}p+\frac{y_2}{x_1}q-\frac{x_2}{x_1}u
		\end{gather*}and show that $v\in\R p+\R q+\R u$.
	\end{enumerate}
\end{proof}
