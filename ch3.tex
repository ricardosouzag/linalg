...
\end{cor}
\begin{proof}
	If there's some $x\in X$ such that $[x\in \spen(X\setminus\{x\})$, then
	\[\spen X=\sum_{x_i\in X}\R x_i=\sum_{x_i\in X\setminus \{x\}}\R x_i+\R x=\spen(X\setminus \{x\})+\R x=\spen (X\setminus \{x\})\]so $X$ is linearly dependent.
\end{proof}